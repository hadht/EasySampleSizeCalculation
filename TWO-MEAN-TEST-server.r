...
res <- ceiling((z.a + z.b) ^ 2 * (input$sd1 ^ 2 + input$sd2 ^ 2) / input$d ^ 2)
}
else {
if (!input$two.sided) f <- function(x) (qt(1 - input$sig.level, x - 1) + qt(input$power, x - 1)) ^ 2 * (input$sd1 ^ 2 + input$sd2 ^ 2) / input$d ^ 2 - x else f <- function(x) (qt(1 - input$sig.level / 2, x - 1) + qt(input$power, x - 1)) ^ 2 * (input$sd1 ^ 2 + input$sd2 ^ 2) / input$d ^ 2 - x
res <- ceiling(uniroot(f, c(2,1e+07), tol = .Machine$double.eps ^ 0.25)$root)
}
paste0("The Sample size is", 2 * res, " individuals (", res, " individuals for each group)."
)
})
})
